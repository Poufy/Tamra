...
func (r *RestaurantRepositoryImpl) CreateRestaurant(restaurant *models.Restaurant) (*models.Restaurant, error) {
	const query = "INSERT INTO restaurants (name, location, logo_url, fb_user_id, created_at, updated_at) VALUES ($1, ST_SetSRID(ST_MakePoint($2, $3), 4326), $4, $5, CLOCK_TIMESTAMP(), CLOCK_TIMESTAMP()) RETURNING id, name, ST_X(location::geometry) as longitude, ST_Y(location::geometry) as latitude, logo_url, fb_user_id, created_at, updated_at"
	err := r.db.QueryRow(query, restaurant.Name, restaurant.Longitude, restaurant.Latitude, restaurant.LogoURL, restaurant.FBUserID).Scan(&restaurant.ID, &restaurant.Name, &restaurant.Longitude, &restaurant.Latitude, &restaurant.LogoURL, &restaurant.FBUserID, &restaurant.CreatedAt, &restaurant.UpdatedAt)
	return restaurant, err
}

func (r *RestaurantRepositoryImpl) GetRestaurant(userId string) (*models.Restaurant, error) {
	restaurant := &models.Restaurant{}
	err := r.db.QueryRow("SELECT id, name, ST_X(location::geometry) as longitude, ST_Y(location::geometry) as latitude, logo_url, fb_user_id, created_at, updated_at FROM restaurants WHERE fb_user_id = $1", userId).Scan(&restaurant.ID, &restaurant.Name, &restaurant.Longitude, &restaurant.Latitude, &restaurant.LogoURL, &restaurant.FBUserID, &restaurant.CreatedAt, &restaurant.UpdatedAt)
...
func (r *RestaurantRepositoryImpl) UpdateRestaurant(restaurant *models.Restaurant) (*models.Restaurant, error) {
	const query = "UPDATE restaurants SET name = $1, location = ST_SetSRID(ST_MakePoint($2, $3), 4326), logo_url = $4, updated_at = CLOCK_TIMESTAMP() WHERE fb_user_id = $5 RETURNING id, name, ST_X(location::geometry) as longitude, ST_Y(location::geometry) as latitude, logo_url, fb_user_id, created_at, updated_at"
	err := r.db.QueryRow(query, restaurant.Name, restaurant.Longitude, restaurant.Latitude, restaurant.LogoURL, restaurant.FBUserID).Scan(&restaurant.ID, &restaurant.Name, &restaurant.Longitude, &restaurant.Latitude, &restaurant.LogoURL, &restaurant.FBUserID, &restaurant.CreatedAt, &restaurant.UpdatedAt)
	return restaurant, err
}

func (r *RestaurantRepositoryImpl) GetRestaurants() ([]*models.Restaurant, error) {
	rows, err := r.db.Query("SELECT id, name, ST_X(location::geometry) as longitude, ST_Y(location::geometry) as latitude, logo_url, fb_user_id, created_at, updated_at FROM restaurants")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
